...
const resolveEndpoint = () => {
  ...
  if (/\/v1\/publico\/imobiliarias$/.test(base)) return base
  if (/\/v1\/publico$/.test(base)) return `${base}/imobiliarias`
  if (/\/v1$/.test(base)) return `${base}/publico/imobiliarias`

  return `${base}/v1/publico/imobiliarias`
}
